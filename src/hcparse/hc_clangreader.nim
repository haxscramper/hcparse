...
#         result = strval.cxxPair().cxxTypeUse()
#         # result = newNimType(strval, cxtype)
...
proc wrapMethods*(
  ...
  for meth in cd.members:
    ...
    res.methods.add wrapProcedure(meth, conf, cache, some cd)

    if meth.cursor.kind in { ckConstructor, ckConversionFunction }:
      res.flags.incl cofExplicitConstructor

    elif meth.cursor.kind in { ckDestructor }:
      res.flags.incl cofExplicitDestructor
      # hasDestructor = true
...
proc wrapApiUnit*(
  ...
  for decl in api.decls:
    ...
    case decl.kind:
      ...
      of cdkForward:
        result.add decl.wrapForward(conf, cache) # GenForward(cdecl: decl, iinfo: currLInfo())
