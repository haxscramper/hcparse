...
proc getValue*(tok: ptr WaveTokenHandle): cstring {.apiProc, importc: "wave_tokGetValue".}
proc getIdRaw*(tok: ptr WaveTokenHandle): cint {.apiProc, importc: "wave_tokGetId".}

proc getId*(tok: ptr WaveTokenHandle): WaveTokId =
  tok.getIdRaw().WaveTokId()

type WaveIteratorHandle* {.apiPtr.} = object

proc getTok*(iter: ptr WaveIteratorHandle): ptr WaveTokenHandle {.
    apiProc, importc: "wave_iterGetTok".}

proc advance*(iter: ptr WaveIteratorHandle) {.
    apiProc, importc: "wave_advanceIterator".}

proc `!=`*(iter1, iter2: ptr WaveIteratorHandle): bool {.
    apiProc, importc: "wave_neqIterator".}
...
proc newWaveContext*(str, filename: cstring): ptr WaveContextHandle {.
    apiProc, importc: "wave_newWaveContext".}

proc destroyContext*(ctx: ptr WaveContextHandle) {.
    apiProc, importc: "wave_destroyContext".}

proc first*(ctx: ptr WaveContextHandle): WaveIteratorHandle {.
    apiProc, importc: "wave_beginIterator".}

proc last*(ctx: ptr WaveContextHandle): ptr WaveIteratorHandle {.
    apiProc, importc: "wave_endIterator".}
